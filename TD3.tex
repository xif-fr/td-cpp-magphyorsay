...
Ajouter des attributs privés stockant la norme et l'argument comme présenté en introduction. Modifier les setters et les constructeurs pour garder sychronisé ces nouveaux attributs polaires avec les attributs carthésiens. Pour éviter de dupliquer le code, on pourra créer une méthode privée \inline{void sync_carth_to_polar()}. On rajoutera un setter \inline{void set_polar(double r, double theta)}, en s'aidant d'une méthode privée \inline{void sync_polar_to_carth()}.\\
...
\begin{correction}
\subsubsection*{Déclaration : fichier \filename{reseau.h}}
...
\subsubsection*{Déclaration : fichier \filename{reseau.cpp}}
...
Pour un réseau hexagonal, on affichera chaque site sous forme d'un triangle, collé à ses trois voisins. Il n'est pas nécessaire de replier le réseau (qui a une forme de parallélogramme), mais si on le fait, on peut obtenir un résultat ressemblant à :
\begin{center}
...
bool ising_metropolis_evolve (Réseau& S, float beta, float B);
\end{minted}
...
int main () {
  ...
  while (true) {
    ...
    for (int n = 0; n < 1000000; n++) {
      ising_metropolis_evolve(S, beta, B);
    }
...
\begin{center}
\includegraphics[width=0.49\linewidth]{TD3/T_sur_Tc_1.0.png}
\end{center}
...
Quand aux fluctuations de magnétisation, elles sont maximales au point critique, ce qui est facile à comprendre : le système est sur le point de se magnétiser spontanément, mais dans la compétition énergie/entropie personne n'arrive à gagner. La moindre fluctuation change fortement la magnétisation. On remarque que le maximum de fluctuations d'énergie se trouve légèrement en dessous de la température critique, ce qui est attendu pour un système de taille fini.
\end{correction}
...
\subsection{[Bonus] Que coûtent l'abstraction et la compilation séparée ?}
